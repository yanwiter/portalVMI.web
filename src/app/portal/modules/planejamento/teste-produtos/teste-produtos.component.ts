...
@Component({
  selector: 'app-teste-produtos',
  templateUrl: './teste-produtos.component.html',
  styleUrl: './teste-produtos.component.scss'
})
export class TesteProdutosComponent {
  ...
  public buildFiltersParams(): string {
    ...
    if (this.filtros.codigoProduto) {
      params.append('codigoProduto', this.filtros.codigoProduto);
    }

    if (this.filtros.dataTeste) {
      const dataFormatada = formatarDataParaDDMMYYYY(this.filtros.dataTeste);
      params.append('dataTeste', dataFormatada);
    }
  ...
  public filtrar() {
    ...
    if (this.filtros.codigoProduto) {
      queryParams.push(`CodigoProduto=${this.filtros.codigoProduto}`);
    }

    if (this.filtros.dataTeste) {
      const dataFormatada = formatarDataParaDDMMYYYY(this.filtros.dataTeste);
      queryParams.push(`DataTeste=${dataFormatada}`);
    }
  ...
  public getActiveFilters(): { label: string, value: string }[] {
    ...
    if (this.filtros.codigoProduto) {
      filters.push({ label: 'Código Produto', value: this.filtros.codigoProduto });
    }

    if (this.filtros.dataTeste) {
      const data = new Date(this.filtros.dataTeste);
      const dia = String(data.getDate()).padStart(2, '0');
      const mes = String(data.getMonth() + 1).padStart(2, '0');
      const ano = data.getFullYear();
      const dataFormatada = `${dia}/${mes}/${ano}`;

      filters.push({ label: 'Data de Teste', value: dataFormatada });
    }
